   ...
   public class RoundButton :System.Windows.Forms.Button
    ...
    Description("Specifies the depth of the button's bevel.")]
      ...
      protected void mouseDown ( object sender, System.Windows.Forms.MouseEventArgs e )
         {
         if (this.RadioGroup == 0 || !this.Checked)
            buttonDown();
         }
      ...
      protected void buttonDown ()
         ...
         buttonPressOffset = 1;
         if (Pushable)
            this.btnChecked = !this.btnChecked; // do not use property
         if (this.Checked && this.RadioGroup > 0)
            {
            foreach (Control C in this.Parent.Controls)
               if (C is RoundButton && C != this)
                  if (((RoundButton) C).RadioGroup == this.RadioGroup)
                     {
                     ((RoundButton) C).Checked = false;
                     ((RoundButton) C).buttonUp();
                     }
